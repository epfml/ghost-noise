...
do
    PYTHONPATH=$PYTHONPATH:$BASE_DIR torchrun --nproc_per_node 1 train.py --output /home/kosson/runs/timm/ --checkpoint-hist 1 \
        ...
        --log-wandb --wandb-kwargs project=nodo_cvmx name="\"test-c10-cvmx-BG3-b0-lr$lr-seed$seed\"" 
    ...
    PYTHONPATH=$PYTHONPATH:$BASE_DIR torchrun --nproc_per_node 1 train.py --output /home/kosson/runs/timm/ --checkpoint-hist 1 \
        ...
        --test-transforms 'PILToTensor={}' \
        -b 256 --opt adamw --lr $lr --weight-decay $wd --sched cosine --sched-on-update --epochs 200 --warmup-epochs 5 --seed $seed --opt-eps=1e-3 \
        --log-wandb --wandb-kwargs project=nodo_cvmx name="\"test-c10-cvmx-BG3-b0-gbs$gbs-seed$seed\"" 
done
